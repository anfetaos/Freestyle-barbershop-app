...
import { Plus, Edit2, Scissors, Search, Loader2 } from 'lucide-react';
import { AppData, Service } from '../types';
import { api } from '../api';
import { formatCurrency, cn } from '../utils';
import { motion } from 'motion/react';

export default function Services({ data, onRefresh, isAdmin }: { data: AppData, onRefresh: () => void, isAdmin: boolean }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedService) {
        await api.editService(selectedService.id, formData);
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filtered.map(service => (
          <div key={service.id} className="glass-card overflow-hidden group">
            <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
              ...
              <div className={cn(
                ...
                service.activo ? "bg-success/10 text-success" : "bg-danger/10 text-danger"
              )}>
            ...
            <div className="p-5">
              <h4 className="text-muted text-[10px] uppercase font-bold tracking-widest mb-1">{service.categoria}</h4>
              <p className="text-lg font-bold text-txt mb-4 truncate">{service.nombre}</p>
              
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-muted uppercase font-bold">{service.duracion} MIN</p>
                  <p className="text-xl font-mono font-bold text-brand-gold">{formatCurrency(service.precio)}</p>
                </div>
                {isAdmin && (
                   <button 
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs uppercase font-bold text-muted mb-2">Duración (min)</label>
                  <input type="number" required value={formData.duracion} onChange={e => setFormData({...formData, duracion: parseInt(e.target.value)})} className="w-full bg-bg border border-white/10 rounded-lg px-4 py-2.5 text-txt outline-none focus:border-brand-blue" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs uppercase font-bold text-muted mb-2">Precio</label>
                  <input type="number" required value={formData.precio} onChange={e => setFormData({...formData, precio: parseFloat(e.target.value)})} className="w-full bg-bg border border-white/10 rounded-lg px-4 py-2.5 text-txt outline-none focus:border-brand-blue" />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="active" className="text-xs uppercase font-bold text-muted">Servicio Activo</label>
              </div>
